...
- **맥락 판정** — 레이어 타입, 자기와 가까운 조상 3단계의 이름, 크기, 불투명도로 9가지 맥락 중 하나를 고른다.
  배지 글자 → 배지 면 → 글자 → 아이콘 → 구분선 → 버튼·입력칸 → 테두리 → 딤 → 면 순서.
  버튼·입력칸 면은 `plain/container/*`, 화면·섹션 바탕은 `plain/background/*` 로 갈린다 — 컴포넌트 라이브러리가 쓰는 기준이다. 이름 규칙(badge·뱃지·tag·chip, icon·ic_·아이콘)은 `code.js`의 `BADGE_RE` · `ICON_RE`.
- **이름이 역할을 말하면 그 역할 안에서** — 옛 BDL 이름은 쓰임을 이름에 담았다(`font/` · `bg/` · `stroke/`). 후보를 그 역할로 좁히고, 노드의 실제 쓰임이 다른 답을 가리키면 이름 기준을 제안하되 확인 필요로 내린다. 답이 같으면 충돌이 아니다.
